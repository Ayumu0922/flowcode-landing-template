...
export default function LogoCloud() {
  return (
    <section data-fc-id="LogoCloud-root" className="py-16 border-y border-zinc-800/50 overflow-hidden">
      <div data-fc-id="LogoCloud-inner" className="w-full">
        <motion.p
          initial={{ opacity: 0 }}
          whileInView={{ opacity: 1 }}
          viewport={{ once: true }}
          data-fc-id="LogoCloud-label"
          className="text-center text-xs font-medium text-zinc-500 uppercase tracking-[0.2em] mb-8 px-6"
        >
          1,000社以上の企業が導入
        </motion.p>
          ...
          <div className="flex animate-marquee">
            {[...logos, ...logos].map((name, i) => (
              <div
                ...
                className="flex items-center justify-center min-w-[180px] mx-8 py-3"
              >
                <span className="text-xl font-bold text-zinc-600 tracking-tight select-none whitespace-nowrap">
                  {name}
                </span>
              </div>
            ))}
